...
const CreatorProfile = () => {
  ...
  const toggleFollow = (id) => {
    fetch(api_base_url + "/toggleFollow", {
      ...
      if (data.success) {
        setIsYouFollowed(data.isFollowed)
      }
      else {
        alert(data.msg)
      }
      ...
      <div className="flex">
        ...
        <div>
          <div className="flex p-[60px] items-center gap-[20px]">
           ...
           {
            isYouFollowed ? <>
            <button onClick={()=>{toggleFollow(id)}} className="btnBlue w-[150px] !rounded-[30px] absolute right-10">Un Follow</button>
            </> : <button onClick={()=>{toggleFollow(id)}} className="btnBlue w-[150px] !rounded-[30px] absolute right-10">Follow</button>
           }
          </div>
