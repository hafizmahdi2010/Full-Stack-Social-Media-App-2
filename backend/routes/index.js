...
router.post("/toggleFollow", async (req, res) => {
  let { followUserId, userId } = req.body;
  // ^ finding the user who hit's the api
  let user = await userModel.findById(userId);
  // ^ Checking that user is exist or not 
  if (user) {
    // ^ if user exist than finding the user which he/she following
    let followUser = await userModel.findById(followUserId);
    if (followUser) {
      // ^ checking that user which are trying to follow the user are in already in following or not.

      if (followUser.followers.some(follower => follower.userId.toString() == userId.toString())) {
        followUser.followers.pull({ userId: userId });
        ...
        followUser.followers.push({ userId: userId });
        await followUser.save();
        return res.json({
          success: true,
      ...
      return res.json({
        success: false,
        msg: "User which you are trying to follow is not exist in database !"
      })
    }
  }
  else {
    return res.json({
      success: false,
      msg: "You are not exist in database !"
    })
  }
});
...
router.post("/getFollowers", async (req, res) => {
  ...
  if (user) {
    ...
    // Use Promise.all to handle async calls inside the map
    fullData = await Promise.all(
      followers.map(async (follower) => {
        return await userModel.findById(follower.userId.toString());
      })
    );
